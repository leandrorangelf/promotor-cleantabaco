...
assert.ok(html.includes('onclick="abrirFotoGaleria(${i})"'), 'fila deve usar indice local sem interpolar visita_id no HTML');

assert.ok(html.includes('async function confirmarTabelaManual'), 'deve existir funcao de confirmacao manual sem IA');
assert.ok(html.includes('confirmarTabelaManualPorIndice(${i})'), 'confirmacao manual deve usar indice local seguro');
